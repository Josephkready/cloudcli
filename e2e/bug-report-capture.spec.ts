...
test('a report filed with the keyboard up records the keyboard, not its dismissal', async ({
  ...
  // The whole point of the three rows. `0px` here is the reporter measuring its
  // own side effect: the tap that opened it took the keyboard away first.
...
test('a report filed with no keyboard still records an honest zero', async ({ page }) => {
  await page.goto('/');
  await expect(page.locator(COMPOSER)).toBeVisible();
  ...
  expect(metadata.keyboardInset).toBe('0px');
