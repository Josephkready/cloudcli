...
1. **Unified envelope**: every server-to-client frame carries a `kind` — either a provider `NormalizedMessage` kind or a gateway kind (`chat_subscribed`, `chat_resumed`, `chat_send_accepted`, `pong`, `session_upserted`, `loading_progress`, `projects_snapshot_stale`, `protocol_error`). Outbound frames never use `type`; the one exception is that a `protocol_error` rejecting an unknown message may echo the offending `type` back (see note 7), which is diagnostic, not a second protocol.
   ...
   **Provider kinds carry an `id`. Gateway kinds do not.** Every provider `NormalizedMessage` is minted by `createNormalizedMessage()` (`server/shared/utils.ts`), which guarantees `id`, `sessionId`, `timestamp` and `provider`. Gateway frames are hand-built by the services in this module and carry only the fields listed for them here — no `id`. That split is load-bearing on the client: only provider kinds are transcript rows, so `PERSISTED_KIND_LIST` in `src/components/chat/hooks/useChatRealtimeHandlers.ts` is an **allow-list** of `MessageKind` values, and anything else is consumed as an event or dropped. A gateway frame that reached the transcript store used to throw on its missing `id` and permanently wedge the session's merged view (#450). **Adding a gateway kind?** It needs no client change to be safe, but adding a *provider* kind means adding it to that allow-list, or it will never appear in a transcript.
