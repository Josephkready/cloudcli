...
export const AskUserQuestionPanel: React.FC<PermissionPanelProps> = ({
  ...
  return (
    <div
      ...
      className={`w-full outline-none transition-[opacity,transform] duration-slow ease-out ${
      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200/80 bg-white shadow-lg dark:border-gray-700/50 dark:bg-gray-800/90 dark:shadow-2xl">
            ...
            <div className="mb-2 flex items-center gap-1">
              {questions.map((_, i) => (
                <button
                  ...
                  className={`h-[3px] rounded-full transition-[width,background-color] duration-base ${
                    i === currentStep
        ...
        <div className="scrollbar-thin max-h-48 overflow-y-auto px-4 pb-2" role={multi ? 'group' : 'radiogroup'} aria-label={q.question}>
          <div className="space-y-1">
            {q.options.map((opt, optIdx) => {
              ...
              return (
                <button
                  ...
                  className={`group flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left transition-[color,background-color,border-color,box-shadow] duration-fast ${
                    isSelected
                ...
                >
                  {/* Keyboard hint */}
                  <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded font-mono text-[10px] transition-colors duration-fast ${
                    isSelected
            ...
            {/* Other text input — inline. The input reserves a right padding
                strip (pr-14) for the absolutely-positioned Enter hint: with
                symmetric px-3 the caret slid under the badge and the tail of a
                long answer was unreadable, worst on narrow viewports (#310). */}
            {isOtherOn && (
          ...
          <div className="flex items-center gap-1.5">
            {!isSingle && !isFirst && (
              <button
                ...
                className="inline-flex items-center gap-0.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-gray-600 transition-colors duration-fast hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700/60"
              >
            ...
            {isLast ? (
              <button
                ...
                className={`inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[11px] font-semibold text-white shadow-sm transition-shadow duration-fast enabled:hover:shadow-md dark:from-blue-500 dark:to-blue-600 ${disabledControlClasses}`}
              >
                Submit
              ...
              <button
                type="button"
                onClick={() => setCurrentStep(s => s + 1)}
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[11px] font-semibold text-white shadow-sm transition-shadow duration-fast hover:shadow-md dark:from-blue-500 dark:to-blue-600"
              >
                Next
                <span className="ml-0.5 font-mono text-[9px] opacity-70">Enter</span>
              </button>
