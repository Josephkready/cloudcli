...
export const ToolDiffViewer: React.FC<ToolDiffViewerProps> = ({
  ...
  return (
    <div className="overflow-hidden rounded border border-gray-200/60 dark:border-gray-700/50">
      ...
      {/* Diff lines. The block scrolls as one surface so the +/- gutter and the
          code stay on the same line while reading a wide diff; `w-max min-w-full`
          lets a row grow past the viewport (so there is something to scroll)
          while still painting its background the full width of a short one. */}
      <div data-scrolls-x className="overflow-x-auto font-mono text-[11px] leading-[18px]">
