...
import { resolveNewBuildAvailable, type ServerBuildIdentity } from '../hooks/buildVersion';
...
/**
 * The shared value every consumer of the version check reads. Deliberately identical to
 * the shape `useVersionCheck` has always returned, so components destructure the same
 * fields they always did.
 */
export interface VersionCheckValue {
  currentVersion: string;
  installMode: InstallMode;
  runningVersion: string | null;
  restartRequired: boolean;
  newBuildAvailable: boolean;
  build: ServerBuildIdentity | null;
...
/**
 * Owns the ONE live `/health` poll for the whole app.
 *
 * Before this provider existed, `useVersionCheck` was called independently by four
 * components (AppContent, Sidebar, AboutTab, BugReportDialog). Each mounted call started
 * its own 60s interval, registered its own `visibilitychange` listener, and kept its own
 * copy of the derived state — so a tab with all four mounted made four requests per
 * interval, and a poll timer that drifted in one component could disagree with the others
 * about whether a new build was available. Hoisting the polling here means exactly one
 * interval and one listener exist for the lifetime of the app; every consumer subscribes
 * to the same state and renders the same answer.
 *
...
export function VersionCheckProvider({ children }: { children: ReactNode }) {
  ...
  // Guards against setting state after unmount while a fetch this provider started is still
  // in flight (a callback-based replacement for the effect-scoped `cancelled` flag this used
...
/**
 * Reads the shared build-identity state owned by `VersionCheckProvider`.
 *
 * Must be used inside the provider. Throwing rather than falling back to a private poll is
 * deliberate: a silent fallback is exactly the multi-poll behaviour this context exists to
 * remove, and it would only resurface when a component is rendered outside the app shell
 * (e.g. an un-wrapped test), where the mistake is cheap to fix.
 */
export function useVersionCheck(): VersionCheckValue {
  const value = useContext(VersionCheckContext);
  if (!value) {
